...
namespace NntpClientLib
{
    ...
    public class Rfc977NntpClientWithExtensions : Rfc977NntpClient
    {
        ...
        public override void Connect(string hostName, int port, bool ssl)
        {
            base.Connect(hostName, port, ssl);
        ...
        public virtual void Connect(string hostName, int port, bool ssl, string userName, string password)
        {
            Open(hostName, port, ssl);
        ...
        {
            NntpReaderWriter.WriteCommand("AUTHINFO USER " + userName, log: false);
            NntpReaderWriter.ReadResponse();
            if (NntpReaderWriter.LastResponseCode == Rfc4643ResponseCodes.PasswordRequired)
            {
                NntpReaderWriter.WriteCommand("AUTHINFO PASS " + password, log: false);
                NntpReaderWriter.ReadResponse();
